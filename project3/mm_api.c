...
struct MM_MapResult MM_Map(int pid, uint32_t address, int writable) {  
    ...
    struct page_table *page_table = (void*)phys_mem; //This is wrong according to Sheahan
    struct page_table_entry *pte = &page_table[virtual_frame_number];

        // init used_pages
    for (int i = 0; i<4; i++){
            page_table -> used_pages[i] = 0;
        }
    ...
    for(int i = MM_ALL_PAGE_TABLES_SIZE_PAGES; i < MM_PHYSICAL_PAGES; i++){
        ...
        if(page_table -> used_pages[i] == 0){
            page_table -> used_pages[i] = 1;
